...
// In dev: __dirname = packages/ezcoder-eyes/src. After build: dist/.
...
const EYES_DIR = resolve(process.cwd(), ".ezcoder/eyes");
...
const GITIGNORE_ENTRIES = [
  ...
  ".ezcoder/eyes/state/",
  ".ezcoder/eyes/bin/",
  ".ezcoder/eyes/recordings/",
  ".ezcoder/eyes/auth/",
  ".ezcoder/eyes/remote.json",
  ".ezcoder/eyes/_lib.sh",
  ".ezcoder/eyes/_redact.sh",
];
...
function doInstall(args: string[]) {
  const cap = args[0];
  if (!cap) {
    console.error("usage: ezcoder-eyes install <capability> [--impl <name>] [--as <script-name>]");
    process.exit(1);
  }
  ...
  // Ensure .ezcoder/eyes exists + shared is present
  doInit();
  ...
  // to .ezcoder/eyes/logs.sh but the manifest recorded .ezcoder/eyes/runtime_logs.sh.
...
function doList() {
  const m = readManifest();
  if (!m.probes.length) {
    console.log("no probes installed. run: ezcoder-eyes detect | install <capability>");
    return;
  }
...
function doLog(args: string[]) {
  const sub = args[0];
  if (!sub) {
    console.error("usage: ezcoder-eyes log <rough|wish|blocked|ack|defer|list|count> ...");
    process.exit(1);
  }
  ...
  if (LOG_KINDS.has(sub)) {
    const reason = args[1];
    if (!reason) {
      console.error(`usage: ezcoder-eyes log ${sub} "<reason>" [--probe <name>]`);
      process.exit(1);
    }
  ...
  if (sub === "ack" || sub === "defer") {
    const id = args[1];
    if (!id) {
      console.error(`usage: ezcoder-eyes log ${sub} <id>`);
      process.exit(1);
    }
...
switch (cmd) {
  ...
  default:
    console.error(
      `usage: ezcoder-eyes <command>

commands:
  init                          create .ezcoder/eyes/, copy shared infra, update .gitignore
  detect                        emit JSON of capability → {candidates, primary}
  install <cap> [--impl <name>] [--as <name>]
                                install deps + copy probe into .ezcoder/eyes/
