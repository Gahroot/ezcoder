...
ggeditor login    # if you don't already use ezcoder
ggeditor
```

Auth lives in `~/.ezcoder/auth.json` — the **same file ezcoder uses**. If you've already logged into ezcoder, ggeditor uses those credentials automatically. No env vars, no API key flags.
...
- The bridge auto-probes the panel — once it's running, ez-editor uses HTTP transport (faster than osascript)
