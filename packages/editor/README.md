...
```bash
npm i -g @prestyj/editor
ezeditor login    # if you don't already use ezcoder
ezeditor
```

Auth lives in `~/.ezcoder/auth.json` — the **same file ezcoder uses**. If you've already logged into ezcoder, ezeditor uses those credentials automatically. No env vars, no API key flags.
...
EZ Editor doesn't replace Resolve or Premiere. It drives them. The agent reads your timeline, makes decisions, leaves markers explaining each cut, and hands the project back to you for finishing. Every editorial decision is auditable — the markers ARE the explanation.
...
```bash
ezeditor "cut all silences from podcast.mp4"
```
...
```bash
ezeditor "from this 2-hour interview, keep only the strongest takes about authentication"
```
...
EZ Editor is built around two video-content workflows:
...
```
ezeditor "cut all silences from /path/to/podcast.mp4"
```
...
```
ezeditor "from /path/to/interview.mp4, find every place I retook a line and pick the best version"
```
...
```
ezeditor "find me the 5 best frames in /path/to/video.mp4 for thumbnails"
```
...
```
ezeditor "show me only the red 'PAUSE' markers between 02:00 and 04:00"
```
...
```
ezeditor "transcribe interview.mp4 with speaker labels"
```
...
```
ezeditor "mix podcast.wav with intro_music.mp3 ducked under the voice"
```
...
```
ezeditor "caption this podcast and attach the SRT to the timeline"
```
...
```
ezeditor "normalize this podcast for Spotify"
```
...
```
ezeditor "add YouTube chapters to this 45-min interview"
```
...
```
ezeditor "cover up the worst three 'um's with cutaways from broll/"
```
...
```
ezeditor "find the best frame in this podcast and make a thumbnail with the headline 'Why most edits fail'"
```
...
```
ezeditor "stitch intro.mp4 + main.mp4 + outro.mp4, normalize for YouTube, burn captions.srt, fade in/out 1s, output final.mp4"
```
...
```
ezeditor "make a 9:16 vertical version of this 5-minute talk"
```
...
```
ezeditor "build a rough cut from script.txt using these 8 source files"
```
...
```
ezeditor "transcribe these 12 mp4s in this folder and emit one EDL per file with silences removed"
```
...
```
ezeditor login                   Authenticate (OAuth for Anthropic / OpenAI; API key for GLM / Moonshot)
ezeditor login --provider <p>    Skip the picker and login to a specific provider
ezeditor logout [provider]       Clear stored credentials (one provider or all)
ezeditor auth                    Show stored credentials + expiry
```
