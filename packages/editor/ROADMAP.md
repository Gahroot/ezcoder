# ez-editor — Roadmap & Outstanding Work
...
- **OAuth login** (`ggeditor login`) — PKCE flow for Anthropic + OpenAI, shared `~/.ezcoder/auth.json` with ezcoder
...
- [x] New package: `@prestyj/editor-premiere-panel`
...
- [x] Dual-transport bridge in ez-editor: HTTP (preferred) + osascript (mac fallback)
...
- [x] `ui/theme.ts` — standalone palette (no ezcoder dep)
...
- [ ] `--resume <session-id>`
- [x] OAuth for Anthropic + OpenAI (PKCE flow, shared auth.json with ezcoder) (currently API key only)
...
- [ ] Comparison matrix: Resolve API vs Premiere API vs ez-editor abstraction
