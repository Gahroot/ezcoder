...
// Mirrors the identical block in ezcoder/src/cli.ts.
...
import { getDefaultModel } from "@prestyj/cli/models";
...
/** Auto-pick order — same priority ezcoder uses for default-provider selection. */
...
function printHelp(): void {
  process.stdout.write(`ezeditor — video editor agent for DaVinci Resolve and Premiere Pro

USAGE
  ezeditor           Launch the editor (auto-detects open NLE)
  ezeditor continue  Resume the most recent session
  ezeditor login     Authenticate
  ezeditor logout    Clear credentials
  ezeditor auth      Show stored credentials
  ezeditor doctor    Walk through environment fixes (--all to view inventory)
  ezeditor logs      Print the path to ~/.ezcoder/ezeditor.log (use with tail -f)

Auth lives in ~/.ezcoder/auth.json — the SAME file ezcoder uses, so logging into
either CLI works for both.

Debug logs are appended to ~/.ezcoder/ezeditor.log — includes Python bridge
spawn/handshake/exit events and every Resolve method call with timing.
Tail it live: tail -f ~/.ezcoder/ezeditor.log
`);
}
...
async function main(): Promise<void> {
  ...
  // ── Bare `ezeditor` (or `ezeditor continue`) — interactive TUI ────────

  if (!process.stdin.isTTY) {
    fail("ezeditor requires an interactive terminal");
  }
  ...
  if (!provider) {
    process.stderr.write(
      chalk.red("Not logged in.\n") +
        chalk.dim("Run: ezeditor login\n\n") +
        chalk.dim(
          "(Auth is shared with ezcoder via ~/.ezcoder/auth.json — log in once, both CLIs work.)\n",
        ),
    );
    process.exit(1);
  }
    ...
    if (!(await hasLastSession())) {
      process.stderr.write(
        chalk.yellow("No previous session to continue.\n") +
          chalk.dim("Run `ezeditor` to start a new one.\n"),
      );
      process.exit(1);
    }
  ...
  // line lands in the log. ~/.ezcoder/ezeditor.log, append-mode across sessions.
