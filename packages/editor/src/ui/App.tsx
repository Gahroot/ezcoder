...
import { useAgentLoop } from "@prestyj/cli/ui/hooks/agent-loop";
...
// Editor brand pulse — amber → orange → rose. Distinct from ezcoder's blue/purple.
...
export interface AppProps {
  ...
  /** Persist session for `ezeditor continue` after each completed turn. */
...
// Names are bare (no `/` prefix) — ezcoder's slash menu prepends `/` for
...
function promptCmd(name: string) {
  ...
  if (!c) throw new Error(`ez-editor: missing prompt command '${name}' — check prompt-commands.ts`);
...
export function App(props: AppProps) {
  ...
  const handleSubmit = useCallback(
    (value: string) => {
        ...
        setHistoryItems((items) => [
          ...items,
          {
            ...
            text:
              ...
              "- `Ctrl-C` (twice) exit\n\n" +
              "**Resume**\n\n" +
              "- `ezeditor continue` — pick up the most recent session\n",
          },
        ]);
        return;
      }
      ...
      // command's prompt as the user message. Mirrors ezcoder's pattern.
  ...
  // exits cleanly. Mirrors ezcoder's pattern.
  const handleDoubleExit = useDoublePress(setExitPending, () => {
    props.onShutdown();
    exit();
  });

  // Toggle thinking via Shift-Tab — same binding ezcoder uses.
  ...
  return (
    <Box flexDirection="column">
      ...
      {/* Model picker overlay sits below the input (replacing the Footer)
          while open — matches ezcoder's layout exactly. */}
      {overlay === "model" ? (
        <ModelSelector
