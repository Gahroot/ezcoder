 ...
 *   not reachable. So unlike the CEP path (panel = HTTP server, ezeditor =
 *   HTTP client), the UXP path **flips the transport**:
 *
 *     - ezeditor (this side) hosts a localhost WebSocket server.
 *     - The UXP plugin connects out to it on launch and stays connected.
 *     - RPC requests flow ezeditor → plugin; responses come back on the same
