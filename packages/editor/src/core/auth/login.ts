 ...
 * every screen ezeditor renders (login selector, status output, login flow
 * messages) so the look is consistent and clearly distinct from ezcoder.
 */
...
/**
 * Interactive `ezeditor login` flow. Uses the shared Ink-based provider
 * selector with the editor's warm palette + brand. Credentials saved to
 * ~/.ezcoder/auth.json — same file ezcoder uses, so logging in via either CLI
 * works for both.
 */
export async function runLogin(opts: { provider?: SupportedAuthProvider } = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    throw new Error("ezeditor login requires an interactive terminal");
  }
  ...
  process.stdout.write(
    orange(`\n✓ Logged in to ${displayName(provider)}.\n`) +
      ...
      chalk.dim("  (Same file ezcoder uses — both CLIs share auth.)\n"),
  );
}
...
export async function runStatus(): Promise<void> {
  ...
  if (providers.length === 0) {
    process.stdout.write(chalk.yellow("  No credentials stored. Run `ezeditor login`.\n"));
    return;
  }
