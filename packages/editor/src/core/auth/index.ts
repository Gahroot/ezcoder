/**
 * ezeditor's auth surface. Single source of truth lives in
 * `@prestyj/cli/auth` \u2014 we re-export it here so editor code keeps a
 * stable import path while the underlying OAuth flows, refresh logic, and
 * AuthStorage are exactly the ones ezcoder ships and exercises daily.
 *
 * Why re-export instead of fork: every previous fork (ezeditor 0.6.0\u20130.6.6)
...
export type { OAuthCredentials, OAuthLoginCallbacks } from "@prestyj/cli/auth";
