...
/**
 * Auth storage backed by ~/.ezcoder/auth.json — the SAME file ezcoder uses, so
 * credentials carry across both CLIs. Logging into one logs into the other.
 *
 * Simplified vs ezcoder's version: no multi-process file-lock (single-CLI
 * use case here) and no logger; otherwise the wire format is identical.
 */

type AuthData = Record<string, OAuthCredentials>;

export const AUTH_FILE = path.join(os.homedir(), ".ezcoder", "auth.json");
