...
function renderFocused(report: DoctorReport, opts: RenderOptions): string {
  ...
  if (!next) {
    ...
    out.push(
      "  " +
        ...
        chalk.dim(" Run `ezeditor` to start."),
    );
    out.push("  " + progressBar(done, total));
    out.push(chalk.dim("  Re-run with `ezeditor doctor --all` to see the full inventory."));
    out.push("");
    return out.join("\n");
  }
  ...
  out.push("");
  out.push(chalk.dim("  Re-run `ezeditor doctor` after fixing to see the next item."));
  if (!opts.all) {
    out.push(chalk.dim("  Or `ezeditor doctor --all` to see the full inventory."));
  }
  out.push("");
  return out.join("\n");
}
...
function renderAll(report: DoctorReport, _opts: RenderOptions): string {
  ...
  if (!next) {
    ...
    out.push(
      "  " + chalk.dim("Next: ") + chalk.bold(next.label) + chalk.dim("  —  see `ezeditor doctor`"),
    );
  }
  out.push("");
  return out.join("\n");
