/**
 * Shared chalk banner used by every non-TUI ezeditor screen
 ...
 * @prestyj/cli/ui/login so all surfaces feel like one product.
 *
...
export interface CliBannerOptions {
  /** Brand label shown next to logo line 1. Default "EZ Editor". */
  brand?: string;
...
export function renderCliBanner(opts: CliBannerOptions): string {
  const brand = opts.brand ?? "EZ Editor";
  const lines: string[] = [];
