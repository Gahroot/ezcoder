 ...
 *
 * The CLI runs onboarding on first launch (no `~/.ezcoder/auth.json` and no
 * `~/.ezcoder/onboarded-ezeditor` marker). The same checks are exposed via
 * `ezeditor doctor` so users can re-run it any time.
 *
 * Design rules:
...
export interface DoctorCheck {
   ...
   * to ~/.ezcoder/api-keys.json. Mutually exclusive with `installable` per
...
const ONBOARDED_MARKER = "onboarded-ezeditor";

export function onboardedMarkerPath(home: string = homedir()): string {
  return join(home, ".ezcoder", ONBOARDED_MARKER);
}
...
function checkOpenAIKey(): DoctorCheck {
  ...
  return {
    ...
    fix: present
      ? undefined
      : "Press Enter to paste your key here — it'll be saved to ~/.ezcoder/api-keys.json.",
  };
}

function checkAnthropicKey(): DoctorCheck {
  // Anthropic auth normally goes through OAuth (~/.ezcoder/auth.json). The env
  ...
  return {
    ...
    unlocks: "Direct Anthropic API auth without OAuth. Most users should `ezeditor login` instead.",
    fix: undefined,
  };
}
...
function checkWhisperX(): DoctorCheck {
  ...
  if (ok && !hfToken) {
    return {
      ...
      fix: "Press Enter to paste your token here — it'll be saved to ~/.ezcoder/api-keys.json.",
    };
  }
  return {
    id: "whisperx",
...
function checkAuthFile(home: string): DoctorCheck {
  const path = join(home, ".ezcoder", "auth.json");
  const exists = (() => {
    try {
  ...
  return {
    ...
    unlocks: "The agent itself. Without auth, ezeditor can't talk to a model provider.",
    fix: exists
      ? undefined
      : "Run `ezeditor login` and pick a provider (Anthropic OAuth recommended; OpenAI / GLM / " +
        "Moonshot also supported). Auth is shared with ezcoder via ~/.ezcoder/auth.json — log in once.",
  };
}
