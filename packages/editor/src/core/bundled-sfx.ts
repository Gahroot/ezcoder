 ...
 * `~/.ezcoder/sfx-cache/<name>.wav` (~50 ms one-time per name; cached forever).
 *
...
export function getSfxCacheDir(): string {
  return resolvePath(homedir(), ".ezcoder", "sfx-cache");
}

/**
 * Resolve an SFX argument:
 *
 *   - Bundled name (e.g. `"whoosh"`) → cache hit at `~/.ezcoder/sfx-cache/whoosh.wav`,
