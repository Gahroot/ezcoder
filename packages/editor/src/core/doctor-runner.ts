...
function bannerLines(onboarding: boolean): string[] {
  ...
  return [
    gradientLine(LOGO_LINES[0]) +
      GAP +
      chalk.hex(PRIMARY).bold("EZ Editor") +
...
function renderScreen(
  ...
  // Footer mirrors ezeditor's main-TUI abort pattern: first Esc / Ctrl+C
...
async function mountSelector(onboarding: boolean): Promise<void> {
  ...
  // Double-press exit (mirrors ezeditor's main TUI). First Esc / Ctrl+C
      ...
      if (ok === total) {
        stdout.write(
          chalk.green("\u2713 Nothing to fix. You're all good.") +
            chalk.hex(TEXT_DIM)(" Run `ezeditor` to start.\n"),
        );
      } else {
        stdout.write(
          chalk.hex(TEXT_DIM)(
            `Doctor closed. ${ok}/${total} ready. Re-run \`ezeditor doctor\` any time.\n`,
          ),
        );
      }
      resolve();
 ...
 * with shell access could `cat ~/.ezcoder/api-keys.json` anyway.
 */
