...
export interface TranscriptSegment {
   ...
   * ez-editor's bundled `transcribe` does not produce speaker labels.
   */
  speaker?: string;
}
...
function transcribeWhisperx(inputPath: string, opts: TranscribeOptions): Promise<Transcript> {
  return new Promise((resolve, reject) => {
    const outDir = mkdtempSync(join(tmpdir(), "ez-editor-whisperx-"));
    const args = [
      inputPath,
      "--model",
...
async function transcribeOpenAI(
  ...
  const boundary = `----ez-editor-${Date.now()}-${Math.random().toString(36).slice(2)}`;
