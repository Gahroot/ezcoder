...
- **OAuth login + shared credentials** — `ggeditor login` runs the same PKCE flow as ezcoder (Anthropic + OpenAI) or prompts for an API key (GLM / Moonshot). Credentials live in `~/.ezcoder/auth.json` — the SAME file ezcoder uses. If you've already logged in via ezcoder, ggeditor picks those credentials up automatically with auto-refresh on expiry. No env vars, no `--api-key` flag (removed). New subcommands: `ggeditor login [--provider <p>]`, `ggeditor logout [provider]`, `ggeditor auth`.
