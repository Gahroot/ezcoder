...
describe("createExitPlanTool", () => {
  ...
  it("rejects empty plan files", async () => {
    ...
    const result = resultToString(
      await tool.execute({ plan_path: ".ezcoder/plans/empty.md" }, mockContext),
    );
  ...
  it("rejects non-existent plan files", async () => {
    ...
    const result = resultToString(
      await tool.execute({ plan_path: ".ezcoder/plans/ghost.md" }, mockContext),
    );
  ...
  it("calls onExitPlan for valid plans and returns the result", async () => {
    ...
    const result = resultToString(
      await tool.execute({ plan_path: ".ezcoder/plans/my-plan.md" }, mockContext),
    );
...
describe("plan mode tool restrictions", () => {
  ...
  describe("write", () => {
    ...
    it("allows writing to .ezcoder/plans/ when plan mode is on", async () => {
      ...
      const result = resultToString(
        await tool.execute(
          { file_path: ".ezcoder/plans/my-plan.md", content: "# Plan\n\nDo stuff." },
          mockContext,
        ),
      );
    ...
    it("creates .ezcoder/plans/ directory if it doesn't exist", async () => {
      ...
      await tool.execute(
        { file_path: ".ezcoder/plans/auto-created.md", content: "# Plan" },
        mockContext,
      );
