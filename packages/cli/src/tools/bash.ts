 ...
 * tail-sliced, and the raw output is offloaded to `~/.ezcoder/tool-output/` so
 * the model can recover the lost portion with `read --offset` instead of
...
export function createBashTool(
  ...
  goalModeRefOrShellOpts?: { current: GoalMode } | ResolveShellOpts,
  ...
  const getSandboxPolicy = getSandboxPolicyArg;
