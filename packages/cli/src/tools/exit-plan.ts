...
const ExitPlanParams = z.object({
  plan_path: z.string().describe("Path to the plan markdown file (must be under .ezcoder/plans/)"),
});
...
export function createExitPlanTool(
  ...
  return {
    ...
    description:
      ...
      "The plan file must be under .ezcoder/plans/. The user will approve, reject with feedback, or cancel.",
