...
import type { MessageProvenance, Provider, ThinkingLevel } from "@prestyj/ai";
...
import type { NolanTurnPayload, AppMarkerPayload, RunOutcome } from "./core/session-manager.js";
import {
...
interface AppSettings {
  ...
  /** Nolan model override keyed by normalized project cwd. */
...
async function main(): Promise<void> {
  ...
  // `deactivate()` hooks. Any can hang, so a deadline guarantees process exit.
  const shutdown = installTerminationHandlers({
    scope: "app-sidecar",
    teardown: async () => {
      clearInterval(parentWatch);
      // Radio playback is app-wide (one stream across all windows).
      stopRadio();
      // Close the ~/.ezcoder progress fs.watch handle.
      progress.dispose();
...
async function createSession(
  ...
  async function ensureNolanSession(): Promise<AgentSession> {
    ...
    const nolanAgent = new AgentSession({
      ...
      // Keeping daemon sessions interactive also lets them share one pooled MCP connection.
      onMcpElicit: elicitations.onElicit,
      // Nolan's bursty, spread-out turns outlast the default cache
  ...
  function handle(
    ...
    if (method === "GET" && url === "/history") {
      ...
      void (async () => {
        ...
        const flushNolan = (count: number): void => {
    ...
    if (method === "POST" && url === "/nolan/prompt") {
      ...
      void readBody(req, res).then(async (raw) => {
        ...
        try {
          ...
          if (pending) await syncNolanModel(pending.provider, pending.model);
        }
      });
      return;
    }
