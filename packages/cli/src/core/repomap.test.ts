...
`,
  ...
  it("uses git dirty files as cold-start active package anchors", async () => {
    const cwd = await makeFixture({
      "packages/cli/src/core/repomap.ts": "export function buildRepoMap() {}\n",
      "packages/cli/src/core/agent-session.ts": "export class AgentSession {}\n",
      "packages/editor/src/core/logger.ts": "export function logger() {}\n",
    });
    await touchFiles(cwd, ["packages/cli/src/core/repomap.ts"], 2_000);
    await touchFiles(cwd, ["packages/editor/src/core/logger.ts"], 1_000);
  ...
  it("prioritizes specific changed files over broad hubs on cold start", async () => {
    const cwd = await makeFixture({
      "packages/cli/src/core/repomap.ts": "export function buildRepoMap() {}\n",
      "packages/cli/src/core/repomap.test.ts": "export const tests = true;\n",
      "packages/cli/scripts/verify-repomap-focus.js": "export async function main() {}\n",
      "packages/cli/src/core/agent-session.ts": "export class AgentSession {}\n",
      "packages/cli/src/tools/index.ts": "export function createTools() {}\n",
      "packages/cli/src/ui/App.tsx": "export function App() { return null; }\n".repeat(4_000),
      "packages/cli/src/cli.ts": "export function main() {}\n",
      "packages/voice/src/dirty.ts": "export const voice = true;\n",
    });
    ...
    expect(paths.indexOf("packages/cli/src/ui/App.tsx")).toBeGreaterThan(2);
    expect(paths.indexOf("packages/cli/src/cli.ts")).toBeGreaterThan(2);
    expect(paths.every((filePath) => !filePath.startsWith("packages/voice/"))).toBe(true);
  });
  ...
  it("prioritizes actively mutated read files over broad dirty files", async () => {
    const cwd = await makeFixture({
      ...
      "packages/cli/src/core/repomap.test.ts": "export const tests = true;\n",
      "packages/cli/src/core/agent-session.ts": "export class AgentSession {}\n",
      "packages/cli/src/core/repomap-context.ts": "export const context = true;\n",
      "packages/cli/src/ui/App.tsx": "export function App() { return null; }\n",
      "packages/cli/src/cli.ts": "export function main() {}\n",
    });
    ...
    expect(
      snapshot.files.findIndex((file) => file.path === "packages/cli/src/ui/App.tsx"),
    ).toBeGreaterThan(2);
    expect(
      snapshot.files.findIndex((file) => file.path === "packages/cli/src/cli.ts"),
    ).toBeGreaterThan(2);
  });
  ...
  it("prefers read-context package over larger unrelated dirty package", async () => {
    const cwd = await makeFixture({
      "packages/cli/src/core/repomap.ts": "export function buildRepoMap() {}\n",
      "packages/cli/src/ui/App.tsx": "export function App() { return null; }\n",
      "packages/voice/src/index.ts": "export const voice = true;\n",
      "packages/voice/src/session.ts": "export const session = true;\n",
      "packages/voice/src/tools.ts": "export const tools = true;\n",
    });
    ...
    expect(snapshot.activeRoots).toEqual(["packages/cli"]);
  ...
  it("keeps active monorepo package files and summarizes unrelated packages", async () => {
    const cwd = await makeFixture({
      "packages/cli/src/core/repomap.ts": "export function buildRepoMap() {}\n",
      "packages/cli/src/ui/app.ts": "export const app = true;\n",
      "packages/editor/src/core/logger.ts":
        "export function logger() {}\nexport function many() {}\n",
      "packages/editor/src/core/other.ts": "export const other = true;\n",
      "packages/boss/src/index.ts": "export const boss = true;\n",
    });

    const { markdown, snapshot } = await buildRepoMap({
      cwd,
      changedFiles: ["packages/cli/src/core/repomap.ts"],
      readFiles: ["packages/cli/src/ui/app.ts"],
      focusTerms: ["repomap"],
      maxFiles: 10,
      now: new Date("2026-01-01T00:00:00.000Z"),
      listGitChangedFiles: async () => [
        "packages/cli/src/core/repomap.ts",
        "packages/editor/src/core/logger.ts",
        "packages/editor/src/core/other.ts",
        "packages/boss/src/index.ts",
      ],
    });
    const paths = snapshot.files.map((file) => file.path);

    expect(paths[0]).toBe("packages/cli/src/core/repomap.ts");
    expect(paths[1]).toBe("packages/cli/src/ui/app.ts");
    expect(paths.every((filePath) => filePath.startsWith("packages/cli/"))).toBe(true);
    ...
    const markdown = renderRepoMap(
      {
        version: 3,
        createdAt: "2026-01-01T00:00:00.000Z",
        files: [file("packages/cli/src/core/repomap.ts", ["buildRepoMap"])],
        directories: [
          { path: "packages/cli/scripts", files: 2 },
          { path: "packages/ai/src", files: 4 },
        ],
        changedFiles: [
        ...
        readFiles: ["packages/cli/src/cli.ts"],
        activeRoots: ["packages/cli"],
        otherDirtyRoots: [],
        stats: {
          indexedFiles: 3,
          shownFiles: 1,
