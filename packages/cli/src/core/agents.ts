...
export async function discoverAgents(options: {
  ...
  // Project agents: {cwd}/.ezcoder/agents/*.md
  if (options.projectDir) {
    const projectAgentsDir = path.join(options.projectDir, ".ezcoder", "agents");
    const projectAgents = await loadAgentsFromDir(projectAgentsDir, "project");
    agents.push(...projectAgents);
  }

  // Global agents: ~/.ezcoder/agents/*.md
  ...
  // Bundled defaults — shipped with ezcoder, user-defined agents with the same
...
export function parseAgentFile(rawInput: string, source: "global" | "project"): AgentDefinition {
  ...
  // with fields a newer EZ Coder understands.
...
// ── Bundled agents ─────────────────────────────────────────
// Shipped with EZ Coder (see ./bundled-agents.ts). Used by /bullet-proof and the
// bundled `bulletproof` skill, and available to any subagent call. User-defined agents with the same name
