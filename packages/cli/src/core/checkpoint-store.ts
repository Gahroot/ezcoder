...
export interface CheckpointStoreOptions {
  sessionId: string;
  cwd: string;
  /** Root for checkpoint storage. Defaults to ~/.ezcoder/checkpoints. Injectable for tests. */
  baseDir?: string;
 ...
 *
 * Snapshots live under `~/.ezcoder/checkpoints/<sessionId>/` (never in the project
...
export class CheckpointStore {
  ...
  constructor(options: CheckpointStoreOptions) {
    const base = options.baseDir ?? path.join(os.homedir(), ".ezcoder", "checkpoints");
