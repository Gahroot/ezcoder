 ...
 *  Autopilot Nolan injects (fix prompts, plan-revision prompts, the post-approval
 *  "implement it now" run). EZ Coder otherwise can't tell an autopilot-injected
...
export const AUTOPILOT_INJECTION_PREAMBLE =
  ...
  "access). State it plainly and stop; Nolan routes it to the user.\n\n" +
  "Your instruction:";
 ...
 *  match-string Nolan's digest uses to label the message as injected. */
...
export interface AutopilotCycleDeps {
  ...
  isCancelled: () => boolean;
  /** Host evidence, independent of Nolan's verdict and reminder budgets. */
