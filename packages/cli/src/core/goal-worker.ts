...
export function buildGoalWorkerSystemPrompt(context: GoalWorkerContext): string {
  ...
  return (
    ...
    "Follow only the assigned Goal task prompt, which is passed as this worker's user message. Keep changes focused, use local/free tools, source_path/docs and — only when the steroids tool is actually available — real-code research, and translate the requested outcome into observable proof: ask what would prove this goal actually worked end-to-end, then create the simplest reliable local/free proof path for the domain. " +
