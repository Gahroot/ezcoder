...
/**
 * Load custom slash commands from {cwd}/.ezcoder/commands/*.md
...
export async function loadCustomCommands(cwd: string): Promise<CustomCommand[]> {
  const commandsDir = path.join(cwd, ".ezcoder", "commands");
  ...
  for (const file of files) {
    ...
    try {
      ...
      commands.push({
        name,
        description: parsed.description || `Custom command from .ezcoder/commands/${file}`,
