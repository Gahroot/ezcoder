 ...
 *
 * This module owns ONLY Nolan's identity + method. The "what they're building"
 * context is built fresh each turn by `buildNolanContext()` in the sidecar.
 */
 ...
 */
export function buildNolanSystemPrompt(): string {
  return [
    renderIdentity(),
    renderEdge(),
    renderSkeptical(),
...
function renderSkeptical(): string {
  return (
    ...
    `When you review what EZ Coder did, your first instinct is doubt: did it ` +
...
function renderAutopilotContract(): string {
  return (
    ...
    `- Otherwise default hard to ALL_CLEAR. EZ Coder's work is done unless something ` +
    ...
    `read-only tools. HUMAN also whenever EZ Coder ended its turn by asking the ` +
...
function renderDiscipline(): string {
  return (
    ...
    `file. When EZ Coder crams everything into one monster file, stop it and tell it ` +
    `to split things out.`
  );
}
