...
export type ProjectSource = "ezcoder" | "claude-code" | "codex" | "folder";

export interface DiscoverProjectsOptions {
  /** The user's configured projects folder; always scanned when set. */
  projectsRoot?: string;
  /** Extra folders explicitly added as project roots. */
...
/**
 * Scan EZ Coder + Claude Code + Codex session stores and the user's project
 * folders, returning one deduplicated row per project, most-recent first.
 */
...
/** Bounded-concurrency Promise.all for large session stores. */
 ...
 *
 * `path.isAbsolute` is platform-bound, but a portable session store can contain
 * Windows drive/UNC paths or POSIX paths regardless of the current host.
 */
...
/**
 * Best-effort decode of an ezcoder session directory name back to a cwd, used
...
/**
 * List the most recent ezcoder conversations for a project cwd. Compaction
 ...
 * SessionManager.list does to count messages), sort files by mtime and parse
 * only enough recent files to return `limit` distinct conversations.
 */
