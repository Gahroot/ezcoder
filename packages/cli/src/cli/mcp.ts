...
function printMcpHelp(): void {
  console.log(`ezcoder mcp — add and manage MCP servers

Usage:
  ezcoder mcp                              Open the interactive dashboard
  ezcoder mcp list                         List servers with live connection status
  ezcoder mcp get <name>                   Show one server's config
...
Add examples:
  ...
  ezcoder mcp add airtable -- npx -y airtable-mcp-server
...
async function runList(cwd: string): Promise<void> {
  ...
  if (rows.length === 0) {
    console.log(dim("No MCP servers configured. Add one with `ezcoder mcp add …`."));
    return;
  }
...
async function runGet(rest: string[], cwd: string): Promise<void> {
  const name = rest[0];
  if (!name) {
    process.stderr.write("Usage: ezcoder mcp get <name>\n");
    process.exit(1);
  }
...
async function runRemove(rest: string[], cwd: string): Promise<void> {
  ...
  if (!name) {
    process.stderr.write("Usage: ezcoder mcp remove <name> [--scope global|project]\n");
    process.exit(1);
  }
...
async function runAdd(rest: string[], cwd: string): Promise<void> {
  ...
  console.log(dim("  Restart ezcoder to load the new server."));
}
...
async function runInteractiveAdd(cwd: string): Promise<void> {
  const line = await promptWithBanner(CLI_VERSION, {
    ...
    question: "Command: ",
    hint: "Paste a `claude mcp add …` or `ezcoder mcp add …` line. Empty to go back.",
  });
  if (line === null) return;
