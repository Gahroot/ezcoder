...
describe("routeCliCommandInput", () => {
  it("prints top-level help before dispatching subcommands", () => {
    const argv = ["node", "ezcoder", "login", "--help"];
  ...
  it("dispatches subcommands and strips command tokens only for modes that parse their own flags", () => {
    const calls: string[] = [];
    const sessionsArgv = ["node", "ezcoder", "sessions", "--foo"];
    const loginArgv = ["node", "ezcoder", "login", "--foo"];
    ...
    expect(sessionsArgv).toEqual(["node", "ezcoder", "--foo"]);
    ...
    expect(loginArgv).toEqual(["node", "ezcoder", "login", "--foo"]);
  ...
  it("converts continue into a later TUI resume route without handling it as a subcommand", () => {
    const argv = ["node", "ezcoder", "continue", "--model", "x"];
    const calls: string[] = [];
    ...
    expect(argv).toEqual(["node", "ezcoder", "--model", "x"]);
