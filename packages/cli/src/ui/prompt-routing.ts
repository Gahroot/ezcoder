...
import type { Provider, TextContent, ImageContent, VideoContent } from "@prestyj/ai";
...
function resolveVideoPath(img: ImageAttachment): string | null {
  ...
  const tmpPath = path.join(os.tmpdir(), `ezcoder-video-${Date.now()}${ext}`);
  try {
...
export function buildUserContentWithAttachments(
  ...
  for (const img of inputImages) {
    if (img.kind === "text") {
      ...
      const tmpPath = path.join(os.tmpdir(), `ezcoder-img-${Date.now()}.${ext}`);
      try {
        writeFileSync(tmpPath, Buffer.from(img.data, "base64"));
        const hint =
          provider === "glm"
