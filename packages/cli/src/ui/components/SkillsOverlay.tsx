...
async function loadSkillEntries(cwd: string): Promise<SkillEntry[]> {
  ...
  // Project skills: {cwd}/.ezcoder/skills/*.md
  await loadFromDir(join(cwd, ".ezcoder", "skills"), "project", entries);

  // Global skills: ~/.ezcoder/skills/*.md
  await loadFromDir(join(homedir(), ".ezcoder", "skills"), "global", entries);
