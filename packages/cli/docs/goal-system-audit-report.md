...
- Reduced redundant planner→setup token handoff in `packages/cli/src/ui/prompt-routing.ts` by stripping the generic slash-command preamble before setup and preserving mandatory references/original user instructions under a concise `## Original Goal Objective` section.
...
- Simplified `buildGoalWorkerSystemPrompt` in `packages/cli/src/core/goal-worker.ts` while preserving proof, worktree isolation, candidate packet, task DAG, evidence, and no-complete contracts for worker-to-worker/coordinator handoff.
...
```sh
pnpm --dir packages/cli exec vitest run src/ui/prompt-routing.test.ts src/ui/slash-command-images.test.ts src/core/goal-worker.test.ts src/system-prompt.test.ts
```

Result: **PASS** — 4 files, 42 tests passed. Note: an earlier `pnpm --filter @prestyj/cli test -- ...` invocation unexpectedly ran broad suites and failed on missing workspace build outputs for `@prestyj/ai`, `@prestyj/agent`, and `@prestyj/pixel`; the focused verifier above avoids that unrelated package-entry failure and directly covers this fix diff.
