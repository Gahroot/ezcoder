...
const evidenceDir = path.resolve("packages/cli/.goal-evidence");
...
try {
  for (const scenario of scenarios) {
    ...
    await execute(tmpProject, refs, {
      ...
      prerequisites,
      harness: [{ id: "scenario-matrix", label: "Domain-agnostic setup/leak contract", command: "pnpm --filter @prestyj/cli exec tsx scripts/verify-goal-scenario-matrix.ts", path: "packages/cli/.goal-evidence/goal-scenario-matrix.log", description: "Checks setup expectations and leak signals across scenario domains." }],
