...
class BackgroundQueue:
    ...
    def _start_worker(self) -> None:
        self._worker = threading.Thread(target=self._run, daemon=True, name="ez-pixel-worker")
        self._worker.start()
        ...
        while attempt < _MAX_ATTEMPTS:
            try:
                ...
                if attempt >= _MAX_ATTEMPTS:
                    print(
                        f"[ez-pixel] dropping event after {_MAX_ATTEMPTS} failed deliveries: {e}",
                        file=sys.stderr,
                    )
                    return
    ...
    def emit_sync(self, event: WireEvent) -> None:
        ...
        try:
            ...
            print(f"[ez-pixel] sync emit failed: {e}", file=sys.stderr)
