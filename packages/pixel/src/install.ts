...
export async function install(opts: InstallOptions = {}): Promise<InstallResult> {
  ...
  const existingKey = readEnvKey(envFilePath, "EZCODER_PIXEL_KEY");
...
const key = process.env.EZCODER_PIXEL_KEY${fallback};
if (key) {
  initPixel({
    projectKey: key,
...
export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    initPixel({
      projectKey: process.env.EZCODER_PIXEL_KEY${fallback},
      sink: { kind: "http", ingestUrl: ${JSON.stringify(`${ingestUrl}/ingest`)} },
    });
  }
}`;
...
initPixel({
  projectKey: process.env.EZCODER_PIXEL_KEY ?? ${JSON.stringify(projectKey)},
  sink: { kind: "http", ingestUrl: ${JSON.stringify(`${ingestUrl}/ingest`)} },
});`,
  );
  upsertPixelBlockInFile(
...
function wireElectron({ projectRoot, pkg, projectKey, ingestUrl }: WiringInput): WiringResult {
  ...
  if (htmlFiles.length > 0) {
    ...
    if (!copyIifeBundle(projectRoot, rendererInitPath)) {
      warnings.push(
        "Could not copy gg-pixel browser IIFE bundle — install @prestyj/pixel and re-run.",
      );
    }
    let wiredAny = false;
...
const PIXEL_HTML_MARKER = "<!-- gg-pixel: auto-wired by ezcoder pixel install -->";
...
export function renderBrowserInitFile(ingestUrl: string, projectKey: string): string {
  return `// gg-pixel init — auto-generated by ezcoder pixel install.
...
async function installGo(ctx: NativeInstallContext): Promise<InstallResult> {
  ...
  writeFileSync(
    initFilePath,
    `// gg-pixel init — auto-generated by ezcoder pixel install.
package main
