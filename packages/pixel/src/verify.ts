...
export interface VerifyOptions {
  ...
  /** Backend root, e.g. "https://ez-pixel-server.buzzbeamaustralia.workers.dev" (no trailing /ingest). */
  ingestUrl: string;
...
async function runChildProbe(args: ChildProbeArgs): Promise<void> {
  const ggDir = join(args.projectRoot, ".ezcoder");
  ...
  const script = `import "@prestyj/pixel";
