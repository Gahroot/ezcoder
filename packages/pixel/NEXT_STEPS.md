...
**Pattern**:
- New `@prestyj/pixel/react-native` entry in the existing npm package
- Hook `ErrorUtils.setGlobalHandler(...)` (RN-specific JS global)
- Send via the existing `HttpSink` (fetch is available in RN)

`ezcoder pixel install` already detects `react-native` in deps and emits a
...
`ezcoder pixel install` is unit-tested for detection across all listed
...
account as the ez-pixel-server (`buzzbeamaustralia` — Ken's account), fetches
to `https://ez-pixel-server.buzzbeamaustralia.workers.dev/ingest` get
**error 1042** (Cloudflare's worker→worker same-account block).

**Fixes:**
1. **Best**: add a custom domain to `ez-pixel-server` (e.g.
...
Current resolution is **client-side**: works only when `ezcoder pixel`
...
1. Add `R2` bucket to `ez-pixel-server` worker
...
- **Multi-tenant auth on the dashboard** — ez-pixel-server has no auth; any
...
`packages/pixel/SPEC.md` is the original design doc. It's been kept
...
- `packages/pixel/src/` for the JS multi-runtime SDK
- Each `packages/pixel-{rs,py,rb,go,swift}/` for native SDKs
- `packages/pixel-server/src/` for the backend
...
1. **Read `packages/pixel/SPEC.md`** for the design vision (one paragraph)
