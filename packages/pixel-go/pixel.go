...
const DefaultIngestURL = "https://pixel-server.ngrout70.workers.dev/ingest"
const userAgent = "ez-pixel-go/4.3.72"
...
// Init initializes ez-pixel. Call once at program start.
// Returns an error if already initialized.
func Init(opts Options) error {
	mu.Lock()
	defer mu.Unlock()
	if active != nil {
		return fmt.Errorf("ez-pixel already initialized")
	}
	if opts.ProjectKey == "" {
...
func (c *client) send(ev wireEvent) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[ez-pixel] send failed: %v\n", err)
		return
	}
