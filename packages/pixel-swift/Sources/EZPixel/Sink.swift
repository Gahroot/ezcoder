...
public final class HttpSink: Sink {
    ...
    public func emit(_ event: WireEvent, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        req.setValue("ez-pixel-swift/4.3.70", forHTTPHeaderField: "user-agent")
        ...
        let task = session.dataTask(with: req) { _, response, error in
            ...
            guard let http = response as? HTTPURLResponse else {
                completion(.failure(NSError(domain: "EZPixel", code: -1)))
                return
            }
    ...
    public func emitSync(_ event: WireEvent) -> Result<Void, Error> {
        ...
        var result: Result<Void, Error> = .failure(NSError(domain: "EZPixel", code: -1))
        emit(event) { r in
            result = r
            semaphore.signal()
        }
