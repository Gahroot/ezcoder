...
const SettingsSchema = z.object({
   ...
   *  capability hint in the prompt that `tool_search` expands on demand
   *  (bench/baseline/14-tool-tiering.mjs measures the token/discovery trade). */
  deferredBuiltinTools: z.boolean().default(true),
  /** Use the `rg` binary for `grep` content search when it is on PATH, falling
   *  back to the in-process scanner otherwise (bench/baseline/15-search-recall.mjs). */
