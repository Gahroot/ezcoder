...
4. Open **Window → UXP Plugins → EZ Editor**
5. Click **Connect**
...
Then **restart Premiere Pro**, open `Window → Extensions → EZ Editor`. The panel should show "listening" and a port number.
...
- Make sure `ezeditor` is running before you click Connect, OR enable "Connect on launch" so the panel keeps retrying
