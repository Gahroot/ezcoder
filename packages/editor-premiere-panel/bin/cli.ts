...
function printHelp(): void {
  process.stdout.write(`ez-editor-premiere-panel — installs the ez-editor CEP panel into Adobe Premiere Pro

USAGE
  ez-editor-premiere-panel install      Install the panel + enable debug mode (one-time setup)
  ez-editor-premiere-panel uninstall    Remove the panel
  ez-editor-premiere-panel status       Show install state
  ez-editor-premiere-panel debug-on     Enable PlayerDebugMode (so unsigned panels load)
...
After install:
  ...
  4. Now run \`ggeditor --host premiere\` from the ez-editor CLI
