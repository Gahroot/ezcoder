...
Defaults: `claude-opus-5-5` for the boss, `claude-sonnet-5` for the workers. Anthropic, OpenAI, GLM, and Moonshot are all supported (anything `ezcoder` supports). Swap mid-session with `/model-boss` and `/model-workers` — your choice persists across restarts.

```bash
ezboss --boss-model claude-opus-5-5 --worker-model claude-sonnet-5
