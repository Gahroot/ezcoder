 ...
 * file at ~/.ezcoder/boss/plan.json, append-on-mutation, per-project grouping
...
async function persist(tasks: BossTask[]): Promise<void> {
  ...
  const next = persistChain.then(async () => {
    ...
    // pid so two ezboss processes don't clobber each other's tmp files.
...
export const tasksStore = {
   ...
   * that were running when ezboss exited, so the worker never finished them
