...
/**
 * `ezboss telegram` — interactive setup wizard. Mirrors `ezcoder telegram` but
 * writes to `~/.ezcoder/boss/telegram.json` so the boss has its own bot identity
 * (distinct from ezcoder's coding bot).
 */
  ...
  try {
    ...
    console.log(
      chalk.hex(COLORS.success)(
        ...
        chalk.hex(COLORS.textDim)("    ezboss serve\n"),
    );
  } finally {
    rl.close();
  }
