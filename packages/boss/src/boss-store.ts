...
import type { ActivityPhase, RetryInfo } from "@prestyj/cli/ui";
import type {
  ContentPart,
  Message,
  Provider,
...
// worker event lives there forever. In `ezboss serve` (24/7 Telegram
 ...
 * ezcoder's partitionCompleted/queueFlush for coalesced tools.
 */
...
export interface BossUiState {
   ...
   * escape Ink's live-area drift — same pattern ezcoder adopted across all
...
export const bossStore = {
   ...
   * that mirrors ezcoder's update notice — flat info text gets lost in
  ...
  startTool(toolCallId: string, name: string, args: Record<string, unknown>): void {
    ...
    // ezcoder's App.onToolStart. A same-name, error-free group absorbs the new
   ...
   * entries — matches ezcoder's onAborted behavior so the user sees the same
