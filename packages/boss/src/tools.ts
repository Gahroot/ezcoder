 ...
 */
export const WORKER_PROMPT_BRIEF = `You're being driven by ezboss, an orchestrator. Your tool usage is already visible to it — don't narrate which tools you ran or recap the request.
...
Notes: <ONE line; only if there's something ezboss must know that the above doesn't capture>
Status: <one of: DONE | UNVERIFIED | PARTIAL | BLOCKED | INFO>

Status meanings (be honest — ezboss routes off this):
...
- BLOCKED: couldn't make progress, needs ezboss to unblock or re-prompt
