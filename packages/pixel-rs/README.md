...
```rust
use ez_pixel::{init_pixel, capture, PixelOptions};

fn main() {
    init_pixel(PixelOptions {
        project_key: std::env::var("EZCODER_PIXEL_KEY").unwrap_or_default(),
        ..Default::default()
    });
...
and posts them to your ez-pixel backend. The previous panic hook is preserved
