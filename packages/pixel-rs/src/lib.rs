//! ez-pixel — universal error tracking, optimized for autonomous coding agents.
//!
...
pub const DEFAULT_INGEST_URL: &str =
    "https://pixel-server.ngrout70.workers.dev/ingest";

#[derive(Clone)]
pub struct PixelOptions {
    /// Project key minted by `POST /api/projects` (or `ezcoder pixel install`).
    pub project_key: String,
    /// Override the ingest URL. Defaults to the public ez-pixel server.
...
/// Initialize ez-pixel. Call this once at the start of your program.
///
...
pub fn init_pixel(options: PixelOptions) -> Result<(), String> {
    ...
    if guard.is_some() {
        return Err("ez-pixel is already initialized; call close_pixel() first".into());
    }
