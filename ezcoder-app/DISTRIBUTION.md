# ezcoder-app distribution (macOS / Windows / Linux)
...
A packaged ezcoder-app must launch from Finder / Explorer / a file manager with **no
...
1. **A per-platform Node runtime** (`scripts/stage-node.mjs`). Downloads the
   ...
   at `src-tauri/binaries/eznode-<target-triple>(.exe)`. Tauri copies it next to
...
```bash
pnpm --filter @prestyj/cli build      # produce dist/app-sidecar.js
pnpm --filter ezcoder-app stage:node             # download + stage the Node runtime
pnpm --filter ezcoder-app bundle:sidecar         # esbuild bundle + copy externals
node ezcoder-app/scripts/smoke-sidecar.mjs       # boot bundled node + sidecar, hit /state
pnpm --filter ezcoder-app tauri build            # full installer
```
