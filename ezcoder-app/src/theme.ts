...
export const theme = {
  ...
  // User text + chip — mirrors the ezcoder TUI's color roles. Shared by the user bubble and the chat input so the
  // "this is you" color reads identically in both places.
  userText: "var(--user-text)",
  userBackground: "var(--user-bg)",

  // Nolan Grout (mentor agent): soft cyan. Used as the FULL text color of Nolan's
  // replies (and the @Nolan active chip in the input), so it must read well as
  // body text on the dark canvas. Distinct from the EZ Coder blue dot and the
  // greener `info` teal: the color IS the only signal that a reply is Nolan's.
  nolan: "var(--nolan)",
} as const;
