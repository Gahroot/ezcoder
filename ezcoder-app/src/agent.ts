...
/** A project task from the ~/.ezcoder-tasks store (the agent's `tasks` tool). */
...
export interface SlashCommand {
  ...
  aliases: string[];
  description: string;
  /** "built-in" prompt template or a user ".ezcoder/commands" custom command. */
...
export async function sendNolanPrompt(text: string): Promise<void> {
  ...
  try {
    ...
    await logError(`agent_nolan_prompt failed: ${String(e)}`);
    throw e;
  }
}

/** Cancel Nolan's in-flight run (does not touch EZ Coder's run). */
export async function cancelNolan(): Promise<void> {
  try {
    await waitForReady();
 ...
 * (removes the provider from ~/.ezcoder/auth.json, including a dual-auth provider's
 * separate OAuth key) so it never depends on the sidecar.
 *
 * `method` disconnects one credential from a provider that holds both; omit it
...
/**
 * Read ezcoder-app settings (projects root folder + whether it was explicitly set).
 *
 * Handled NATIVELY in Rust (reads ~/.ezcoder/ezcoder-app.json directly) so the home
...
/**
 * Save ezcoder-app settings. Handled NATIVELY in Rust (writes ~/.ezcoder/ezcoder-app.json
