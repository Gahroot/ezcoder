...
function App(): React.ReactElement {
  ...
  // LLM-generated session title shown in the titlebar ("EZ Coder" until set).
  ...
  // Drive the native OS title bar with the session title or EZ-branded mode name.
  useEffect(() => {
  ...
  // Keyboard shortcuts (global, nav-independent so they stay reachable even when
  // the nav row is collapsed):
  //   Cmd/Ctrl+N         → new project window
  //   Cmd/Ctrl+T         → toggle the Tasks modal (mirrors the CLI's Ctrl+T pane)
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent): void => {
      ...
      // Toggle Tasks modal: Cmd/Ctrl + T (no Shift/Alt).
  ...
  const nolanActive = workspaceMode === "code" && /^@nolan\b/i.test(input.trimStart());
  ...
  // ── Plan review actions (mirror the ezcoder CLI plan overlay) ──
      ...
      <div className="chat-head">
        ...
        <div className="chat-head-strip" data-tauri-drag-region>
          ...
          <span className="chat-head-title" data-tauri-drag-region>
            {sessionTitle ?? (workspaceMode === "chat" ? "EZ Chat" : "EZ Coder")}
          </span>
              ...
              <span className="model-anchor">
                {modelMenuOpen && models.length > 0 && (
                  <ModelMenu
                    ...
                    title={workspaceMode === "chat" ? "EZ Chat model" : "EZ Coder model"}
                  />
                )}
