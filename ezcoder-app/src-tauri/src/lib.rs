...
///
/// Read it through `current_page`, never directly: the stored value can point
/// past the last page after windows close, and every "is this window on
/// screen?" decision in the app is derived from this number rather than from
/// `is_visible()`.
...
fn restore_sibling_windows(window: &tauri::Window) {
    ...
    let siblings: Vec<WebviewWindow> = {
        ...
        for (sib_label, win) in app.webview_windows() {
            ...
            // Off-page windows stay down: restoring them would show them.
...
/// Command substrings that identify an EZ Coder *sidecar* process itself.
...
/// sidecar's parent is the still-running `ezcoder-app`, so its `ppid` is alive, and
...
/// Proxy: list this project's task list (the ~/.ezcoder-tasks store for its cwd).
#[tauri::command]
...
/// Proxy: read ezcoder-app settings (e.g. the projects root folder).
#[tauri::command]
...
/// Proxy: save ezcoder-app settings.
#[tauri::command]
...
const AUTH_PROVIDERS: &[ProviderMeta] = &[
    ProviderMeta {
        value: "anthropic",
        label: "Anthropic",
        description: "Claude Fable 5.1, Opus 5.5, Opus 5, Sonnet 5, Haiku 4.5",
        methods: &["oauth"],
        oauth_key: None,
        oauth_label: None,
        method_details: &[],
    ...
    ProviderMeta {
        value: "openai",
        label: "OpenAI",
        description: "GPT-6 Astra, GPT-6 Sol, GPT-6 Luna, GPT-5.6 Sol, GPT-5.6 Terra, GPT-5.6 Luna",
        methods: &["oauth"],
        oauth_key: None,
        oauth_label: None,
        method_details: &[],
...
/// its variant keys). Never needs the sidecar.
///
/// Also reports which methods are connected and which one a request would use,
/// mirroring AuthStorage's OAuth-first resolution and API-key fallback while
/// subscription OAuth usage is exhausted.
#[tauri::command]
...
/// Native: disconnect a provider from ~/.ezcoder/auth.json.
/// `method` ("oauth" | "apikey") disconnects one of a dual-auth provider's
/// credentials; omitted, it removes all of them, including every API-key
/// variant. Never touches the sidecar. Returns `{ ok: true }`.
#[tauri::command]
fn app_auth_logout(
    app: tauri::AppHandle,
    provider: String,
...
/// Proxy: discover known projects across ezcoder/Claude Code/Codex stores.
#[tauri::command]
...
fn queue_page(app: &tauri::AppHandle, page: u8, tile_count: Option<usize>, focus: Option<String>) {
...
/// which Tauri places next to the app executable named `eznode` (`.exe` on
...
fn restore_or_default_windows(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    if !any_geometry {
        // Only page 1 is tiled. `arrange_windows` works from the page map (not
        // from `is_visible()`, which would still be reporting `false` for the
        // `show()` calls issued microseconds ago), so `count` here is a ceiling,
        // not the grid size.
    ...
    tauri::Builder::default()
            ...
            tauri_plugin_log::Builder::new()
                ...
                .target(tauri_plugin_log::Target::new(
                    tauri_plugin_log::TargetKind::LogDir {
                        file_name: Some("ezcoder-app".into()),
                    },
                ))
                .build(),
        )
        ...
        .on_window_event(|window, event| match event {
            tauri::WindowEvent::Destroyed => {
                ...
                // visible page — or retire the visible page entirely, which
                // would leave the user staring at a screen with no windows on
                // it. Re-apply the page so what's on screen matches.
                //
            ...
            //
            // This deliberately does NOT change the page. Focus is an OUTPUT of
            // a page switch, never an input to one: hiding the focused window
            // makes the window server hand key status to some other window —
            // routinely one on the page being hidden — and an earlier version
            // that followed that focus back to its page ping-ponged the two
            // pages on screen for as long as the events kept arriving.
            // `apply_page` places focus explicitly instead, and the tray already
            // targets a window on the current page (`app_window_labels` is
            // page-filtered), so nothing needs to follow focus across pages.
...
mod tests {
    ...
    fn pick_node_bundled_uses_exe_dir_when_present() {
        let tmp = std::env::temp_dir().join(format!("eznode-test-{}", std::process::id()));
    ...
    fn pick_cwd_release_uses_home_not_build_path() {
        ...
        let got = pick_cwd(
            None,
            false,
            PathBuf::from("/Users/runner/work/ezcoder/ezcoder"),
            PathBuf::from("/home/user"),
        );
    ...
    fn multi_instance_concurrent_dev_runs_safe() {
        // Two ezcoder-app instances each with their own live sidecar. Both sidecar
    ...
    fn parse_ps_handles_column_padding_and_spaces_in_command() {
        ...
        let raw = "    1     0     1 /sbin/launchd\n\
                   11541     1 11541 /Applications/EZ Coder.app/Contents/MacOS/ezcoder-app\n\
                   11553 11541 11553 /Applications/EZ Coder.app/Contents/MacOS/eznode app-sidecar.mjs";
