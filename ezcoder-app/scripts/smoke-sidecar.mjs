...
/** Locate the staged eznode binary (named with the host target triple). */
function nodeBin() {
  const triple = execFileSync("rustc", ["--print", "host-tuple"], {
    encoding: "utf8",
  }).trim();
  const ext = process.platform === "win32" ? ".exe" : "";
  const expected = join(binDir, `eznode-${triple}${ext}`);
  if (existsSync(expected)) return expected;
  // Fallback: any eznode-* in the binaries dir.
  const found = existsSync(binDir)
    ? readdirSync(binDir).find((f) => f.startsWith("eznode-"))
    : undefined;
